...
function Heatmap({ tab }) {
      ...
      {HEATMAP_HOURS.map((h, hi) => (
        <div key={h} className="grid gap-1 mb-1 items-center" style={{ gridTemplateColumns:'36px repeat(7, 1fr)' }}>
          ...
          {HEATMAP_DAYS.map((d, di) => {
            ...
            return <div key={d} className="rounded h-5 cursor-pointer hover:opacity-75 transition-opacity" style={{ background: color(op) }} />;
          })}
        </div>
      ))}
    </div>
  ...
  if (tab === 'Month') return (
    <div>
      ...
      {HEATMAP_WEEKS.map((w, wi) => (
        <div key={w} className="grid gap-1 mb-1.5 items-center" style={{ gridTemplateColumns:'40px repeat(7, 1fr)' }}>
          <div className="text-right pr-1.5 text-[10px] text-brand-400 font-semibold leading-tight">{w}</div>
          {HEATMAP_DAYS.map((d, di) => {
            const op = Math.min(0.1+(di>=5?0.45:0)+seed(wi,di)*0.3, 0.92);
            return <div key={d} className="rounded h-7 cursor-pointer hover:opacity-75 transition-opacity" style={{ background: color(op) }} />;
          })}
        </div>
      ))}
    </div>
  ...
  return (
    <div className="grid grid-cols-4 gap-1.5">
      {HEATMAP_MONTHS.map((m, mi) => {
        const isPeak = [5,6,10,11].includes(mi);
        const op = Math.min(0.1+(isPeak?0.55:0)+seed(mi,3)*0.2, 0.92);
        const light = op < 0.45;
        return (
          <div key={m} className="rounded-lg h-9 flex items-center justify-center text-[11px] font-bold cursor-pointer hover:opacity-75 transition-opacity"
            style={{ background: color(op), color: light ? '#5C3317' : '#fff' }}>
            {m}
          </div>
        );
      })}
...
export default function AnalyticsPage() {
  ...
  const [htTab,   setHtTab]  = useState('Week');
  ...
  const fetchGoogleAIInsights = async (forceRefresh = false) => {
    ...
    try {
      ...
      const systemPrompt = `
        Ikaw ay Business Analyst para sa isang bake shop. Suriin ang data na ito.

        TIMEFRAME NG DATA: Ito ay isang ${timeframePinas} na report. Ibig sabihin, ang data sa ibaba ay para sa kasalukuyang ${view.toLowerCase()}.

        HISTORICAL PERFORMANCE:
        - Sales: ₱${kpi.sales} (Delta: ${kpi.sDelta}%)
        - Expenses: ₱${kpi.expenses} (Delta: ${kpi.eDelta}%)
        - Margin: ${kpi.margin}%
        - Top Products: ${topProds.map(p => p.name).join(', ')}

        FORECAST / PREDICTIONS (Panghinaharap na Projection):
        - Inaasahang TATAAS ang benta: ${growthItems}
        - Inaasahang BABABA ang benta (At Risk): ${riskItems}

        Gumawa ng insights sa WIKANG TAGALOG. Gamitin ang forecast data para magbigay ng payo (halimbawa, ano ang dapat gawin sa mga at-risk na produkto, at paano pa palalakasin ang mga nag-grow). 
        RETURN ONLY A VALID JSON OBJECT:
        {
          "descriptive": "Bumuo ng 2-3 sentences na summary ng nakaraang performance at ano ang sinasabi ng forecast para sa susunod na araw/linggo/buwan.",
          "prescriptive": [
  ...
  const htSubs  = { Week:'Peak hours by day of week', Month:'Peak days per week', Year:'Peak months of the year' };
      ...
      <Card className="p-5 bg-white border border-brand-100 rounded-xl">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-brand-400 mt-0.5">Sales, expenses at profit · {VIEW_PERIOD_LABEL[view]}</p>
            </div>
          </div>
          <div className="flex items-center gap-4 text-xs text-brand-500 font-semibold">
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-blue-500 inline-block"/><span>Sales</span></span>
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-rose-500 inline-block"/><span>Expenses</span></span>
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-emerald-500 inline-block"/><span>Profit</span></span>
          </div>
        </div>
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={chartData} margin={{ top:8, right:12, left:0, bottom:0 }}>
            ...
            <Line type="monotone" dataKey="Sales"    name="Sales"    stroke="#3b82f6" strokeWidth={2.5} dot={{ r:4, fill:'#fff', strokeWidth:2 }} activeDot={{ r:6 }} />
            <Line type="monotone" dataKey="Expenses" name="Expenses" stroke="#f43f5e" strokeWidth={2.5} dot={{ r:4, fill:'#fff', strokeWidth:2 }} activeDot={{ r:6 }} />
            <Line type="monotone" dataKey="Profit"   name="Profit"   stroke="#10b981" strokeWidth={2.5} dot={{ r:4, fill:'#fff', strokeWidth:2 }} activeDot={{ r:6 }} />
          </LineChart>
        </ResponsiveContainer>
      </Card>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card className="p-5 bg-white border border-brand-100 rounded-xl">
          <div className="flex items-start justify-between mb-4">
            ...
            <Tabs value={htTab} onChange={setHtTab} options={HT_VIEWS} />
          </div>
          <Heatmap tab={htTab} />
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-brand-100">
          ...
          <div className="space-y-4">
            {topProds.map((p, i) => (
              <div key={p.name} className="flex items-center gap-3">
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-[13px] font-bold shrink-0 ${rankCls[i]}`}>{i+1}</div>
                <span className="text-[15px] text-brand-800 font-semibold flex-1 truncate">{p.name}</span>
                <div className="w-24 h-2.5 bg-brand-100 rounded-full overflow-hidden shrink-0">
                  <div className="h-full bg-blue-500 rounded-full" style={{ width:`${(p.sold/maxSold*100).toFixed(0)}%` }} />
                </div>
                <span className="text-[15px] font-bold text-brand-900 min-w-[56px] text-right tabular-nums">
                  {p.sold.toLocaleString()} <span className="text-[11px] text-brand-500 font-normal">pcs</span>
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="mt-8 pt-6 border-t-2 border-brand-100 border-dashed">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-2xl font-black text-brand-900 tracking-tight">AI Business Insights & Forecast</h2>
              <p className="text-[14px] text-brand-500 font-medium">Automated data analysis at projections para sa {VIEW_PERIOD_LABEL[view]}.</p>
            </div>
          </div>
        ...
        {isAiLoading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            {aiData && (
              <div className="lg:col-span-2 bg-gradient-to-br from-brand-50/80 to-white border border-brand-200 rounded-2xl p-6 shadow-sm relative overflow-hidden group">
                <div className="relative z-10">
                  ...
                  <p className="text-[16px] text-brand-800 leading-relaxed font-medium max-w-6xl">
                    {aiData.descriptive}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-white border border-brand-200 rounded-2xl shadow-sm flex flex-col p-5">
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 className="text-[13px] font-black text-brand-800 uppercase tracking-widest leading-none mb-1.5">Smart Forecast Analysis</h3>
                  <p className="text-[11px] font-semibold text-brand-500 flex items-center gap-1.5">
                    Timeframe: 
                    <span className="text-brand-800 font-bold bg-brand-100 px-2 py-0.5 rounded-md">
                      {view === 'Day' ? 'Pang-Bukas (Next Day)' : view === 'Week' ? 'Susunod na Linggo (Next Week)' : view === 'Month' ? 'Susunod na Buwan (Next Month)' : 'Susunod na Taon (Next Year)'}
                    </span>
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-6">
                {/* Pataas ang Benta */}
                <div>
                  <h4 className="text-[11px] font-bold text-emerald-700 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-emerald-500" /> Pataas ang Benta (Growth)
                  </h4>
                  <div className="space-y-2.5">
                    {ANALYTICS.predictive.growthLeaders[vk].map((item, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center justify-between p-3 border border-emerald-100 bg-emerald-50/20 rounded-xl hover:shadow-md transition-all">
                          <div className="min-w-0 flex-1">
                            <p className="text-[14px] font-bold text-brand-900 truncate">{item.name}</p>
                            <p className="text-[11px] font-semibold text-emerald-600 mt-0.5">+{calculatedPcs} pcs projected</p>
                          </div>
                          <div className="text-right flex items-center gap-3">
                            <MiniSparkline trend="up" />
                            <span className="inline-block px-2.5 py-1 bg-emerald-100 text-emerald-800 text-[11px] font-extrabold rounded-lg">+{item.pct}%</span>
                          </div>
                        </div>
                      );
                    })}
                ...
                {/* Medyo Matumal */}
                <div>
                  <h4 className="text-[11px] font-bold text-rose-700 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-rose-500" /> Medyo Matumal (At Risk)
                  </h4>
                  <div className="space-y-2.5">
                    {ANALYTICS.predictive.atRisk[vk].map((item, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center justify-between p-3 border border-rose-100 bg-rose-50/20 rounded-xl hover:shadow-md transition-all">
                          <div className="min-w-0 flex-1">
                            <p className="text-[14px] font-bold text-brand-900 truncate">{item.name}</p>
                            <p className="text-[11px] font-semibold text-rose-600 mt-0.5">-{calculatedPcs} pcs projected</p>
                          </div>
                          <div className="text-right flex items-center gap-3">
                            <MiniSparkline trend="down" />
                            <span className="inline-block px-2.5 py-1 bg-rose-100 text-rose-800 text-[11px] font-extrabold rounded-lg">{item.pct}%</span>
                          </div>
                        </div>
                      );
                    })}
            ...
            {/* COLUMN 2: ACTIONABLE RECOMMENDATIONS */}
            {aiData && (
              <div className="bg-white border border-brand-200 rounded-2xl shadow-sm flex flex-col">
                <div className="px-5 py-3.5 bg-slate-50 border-b border-brand-100 flex items-center gap-2 rounded-t-2xl">
                  <Sparkles size={16} className="text-brand-600" />
                  <h3 className="text-[12px] font-black text-brand-800 uppercase tracking-widest">Actionable Recommendations</h3>
                </div>
                <div className="p-5 flex flex-col gap-4">
                  {aiData.prescriptive.map((ins, i) => {
                    const theme = AI_COLORS[ins.type] || AI_COLORS.neutral;
                    return (
                      <div key={i} className="relative bg-white border border-slate-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-all group overflow-hidden" style={{ borderLeft: `4px solid ${theme.color}` }}>
                        <div className="relative z-10">
                          <span className="text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md mb-2 inline-block" style={{ background: theme.tagBg, color: theme.tagTxt }}>
                            {ins.badge}
                          </span>
                          <h4 className="text-[14px] font-bold text-brand-900 mb-1.5 leading-snug">{ins.title}</h4>
                          <p className="text-[13px] text-brand-600 leading-relaxed">{ins.desc}</p>
                        </div>
                      </div>
                    );
                  })}
